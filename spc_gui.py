...
class SpcGui:
    ...
    def build_ui(self):
        ...
        log_frame = LabelFrame(left, text="Log", padx=5, pady=4)
    ...
    def preview_section(self, parent):
        ...
        self.preview_label = Label(frame, text="PNG preview will appear here", anchor="center")
        self.preview_label.pack(fill=BOTH, expand=True, pady=6)

        controls = Frame(frame)
        controls.pack(fill=X)
        Button(controls, text="Load PNG folder", command=self.choose_preview_folder).pack(side=LEFT)
    ...
    def auto_background_from_sheet(self):
        ...
        selected = []
        missing = []
        tif_paths = tif_files(self.experiment_dir)
        ...
        if not selected:
            raise FileNotFoundError("No matching background tif files found in experiment folder.")
    ...
    def show_preview(self):
        if not self.preview_images:
            ...
            self.preview_label.configure(image="", text="PNG preview will appear here")
            self.preview_photo = None
            return

        image_path = self.preview_images[self.preview_index]
        image = Image.open(image_path)
        image.thumbnail((500, 620))
    ...
    def calculate_background(self):
        ...
        self.log(f"Reading {len(moved_files)} moved background files")
        stack = np.array([read_tif(path) for path in moved_files])
